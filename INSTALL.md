...
writing. This installs distributed source templates, not the installing agent's own
Collective governance. The user's scope and prior choices govern this workflow.
...
- If the target is DB-managed/generated, do not overwrite it, even with a backup.
  Use the existing governance tools and COLLECTIVE.md. Do not install file memory there.
...
- **Managed file or managed skill:** use its owning management tool; never route around
  a self-modification denial with a different file-writing command.
...
3. For the direct case, prepare the 2.0 Claude entry with that block, plus PROTOCOL.md as
...
selection change or permission change is part of deployment. COLLECTIVE.md is a reference
in the kit, not a dependency that every installed skill must find beside itself.

For an explicitly named fresh **files** project, copy project-template/ without replacing
...
frontmatter, and no unrelated file changed. Confirm the selected memory backend and
preservation of all existing project records.

Start a fresh user session for the host to discover installed instructions and skills.
Ask it to name the loaded sources and backend and invoke session-start in the intended
