...
- Both install modes preserve existing content and provide a complete loading chain.
...
overwrite a real user home, consume paid APIs or launch untracked workers for a smoke test.

| Scenario | Expected observable outcome |
|---|---|
| Fresh Codex, files mode | AGENTS + shared protocol identified; seven skills discoverable; session-start reads/scaffolds only this project's memory |
| Fresh Claude, files mode | Native entry loads protocol; project CLAUDE reads AGENTS; same records used |
| Existing task in startup request | Context restored, task registered/claimed and work continues without another approval gate |
...
| Collective hooks absent | Missing brief/checkpoint fetched; existing wrapper identity reused; no docs memory created |
| Empty birth headings | Treated as missing context, not a successful bootstrap |
| MCP write fails | No success claim, no silent file fallback; unsaved handoff reported |
...
| Managed global target | No template overwrite; changes routed through the owning governance tools |
